...
const FilialList = () => {
    ...
    const [prefix, setPrefix] = useState(''); // Состояние для префикса
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await addFilial(filialText, userPhone, prefix);
        ...
        <ul>
          {filials.map((filial) => (
            <div className="filial-el" key={filial.filial._id}>
                  <p>Название филиала: <br /> {filial.filial.filialText} <br /> <br /> Номер телефона: {filial.filial.userPhone} <br /> <br /> Префикс: {filial.filial.prefix}</p>
